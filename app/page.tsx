...
import { HOLES, TOTAL_PAR } from '@/lib/course'
import { strokesPerHole, formatVsPar } from '@/lib/scoring'
...
function buildLeaderboard(players: Player[], scores: Score[], foursomes: Foursome[]): PlayerRow[] {
  const foursomeMap = Object.fromEntries(foursomes.map(f => [f.id, f]))
  return players
    .map(player => {
      ...
      const parPlayed = HOLES.filter(h => playerScores.some(s => s.hole_number === h.hole))
...
export default function LeaderboardPage() {
  ...
  if (!roundId) return (
    <div className="text-center mt-16 space-y-4">
      <p className="text-gray-400">No round initialized yet.</p>
      <Link href="/setup" className="bg-green-700 px-6 py-3 rounded-lg font-semibold inline-block">
        Go to Setup
      </Link>
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 rounded-xl overflow-hidden">
        ...
        {rows.map((row, i) => (
          <div
            key={row.player.id}
            className={`grid grid-cols-[2rem_1fr_3rem_3rem_3rem] gap-x-2 px-4 py-3 border-b border-gray-800 last:border-0 ${i === 0 && row.holesPlayed > 0 ? 'bg-yellow-950' : ''}`}
          >
            ...
            <div className="self-center">
              ...
              <p className="text-xs text-gray-500">
                Grp {row.foursome?.group_number ?? '?'} · Hcp {row.player.handicap_index}
              </p>
            </div>
            ...
            <span className="text-right text-sm self-center">
              {row.holesPlayed > 0 ? row.netTotal : '-'}
            </span>
      ...
      {/* Foursome links */}
      <h2 className="text-lg font-bold mt-8">Foursomes</h2>
      <div className="grid grid-cols-3 gap-3">
        {foursomes.map(f => (
          ...
          >
            ...
            <p className="text-xs text-gray-400 mt-1">Vegas + Scores</p>
          </Link>
        ))}
      </div>
    </div>
